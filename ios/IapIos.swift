...
typealias IapIosPromise = (RCTPromiseResolveBlock, RCTPromiseRejectBlock)
...
@objc(IapIos)
class IapIos: RCTEventEmitter, SKRequestDelegate, SKPaymentTransactionObserver, SKProductsRequestDelegate {
  private var promisesByKey: [String: [IapIosPromise]]
  ...
  override init() {
    promisesByKey = [String: [IapIosPromise]]()
  ...
  func addPromise(forKey key: String, resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) {
    var promises: [IapIosPromise]? = promisesByKey[key]
  ...
  func resolvePromises(forKey key: String?, value: Any?) {
    let promises: [IapIosPromise]? = promisesByKey[key ?? ""]
  ...
  @objc public func initConnection(
    ...
    SKPaymentQueue.default().remove(IapIosQueue.shared)

    if let queue = IapIosQueue.shared.queue, let payment = IapIosQueue.shared.payment, let product = IapIosQueue.shared.product {
      let val = paymentQueue(queue, shouldAddStorePayment: payment, for: product)
      debugMessage("Promoted product response \(val)")
    }
